...
public interface ProductRepository extends CrudRepository<Product,Long> {
    //@Query("SELECT p FROM Product p WHERE p.price >= ?1 AND p.price <= ?2")//SIMILAR TO SQL, NOT THE SAME
    @Query("SELECT p FROM Product p WHERE p.price BETWEEN ?1 AND ?2")
    List<Product> findProductByPriceInRange(BigDecimal minPrice, BigDecimal maxPrice); //query notation
